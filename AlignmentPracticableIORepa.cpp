...
//   [VariableRepa] -> HistoryRepa -> HistoryRepa-> Integer ->
//   IO (SystemRepa, FudRepa, [(Double, [VariableRepa]])
std::tuple<std::unique_ptr<FudRepa>, std::unique_ptr<DoubleSizeListPairList>> Alignment::parametersSystemsLayererMaxRollByMExcludedSelfHighestLogIORepa_up(std::size_t wmax, std::size_t lmax, std::size_t xmax, std::size_t omax, std::size_t bmax, std::size_t mmax, std::size_t umax, std::size_t pmax, std::size_t tint, const SizeList& vv, const HistoryRepa& hr, const HistoryRepa& hrs, std::size_t f, void (*log)(const std::string&), SystemRepa& ur)
{
	auto hrred = [](double f, const HistoryRepa& hr, const SizeList& kk)
	{
		return setVarsHistoryRepasReduce_u(f, kk.size(), kk.data(), hr);
...
//  Integer ->
//   [VariableRepa] -> [VariableRepa] -> Integer ->
//   IO (SystemRepa, ApplicationRepa)
std::unique_ptr<ApplicationRepa> Alignment::parametersSystemsHistoryRepasApplicationerCondMultinomialMultiLabelFmaxIORepa_u(std::size_t fmax, const SizeList& vv,  const SizeList& ll, const HistoryRepa& hr0, int d, SystemRepa& ur)
{
	auto hrsel = eventsHistoryRepasHistoryRepaSelection_u;
	auto frmul = historyRepasFudRepasMultiply_u;
	...
	}
	{
		bool any = false;
		for (auto v : vv)
			for (std::size_t i = 0; !any && i < m; i++)
				any = v == ll[i];		
		if (any)
		{
			std::cout << "contains label" << std::endl;
			std::cout << "<<< applicationer " << ((sec)(clk::now() - t0)).count() << "s" << std::endl;
			return dr;
		}
	}
	...
	std::size_t f = 1;
	{
		...
		double e = e0;
		std::size_t i1 = 0;
		auto ee1 = new std::size_t[ns*ls];
		auto ee2 = new std::size_t[ns];
	...
	while (f < fmax)
	{
		...
		std::cout << "fud slice cardinality: " << llt.size() << std::endl;
		time["transer"] = ((sec)(clk::now() - mark)).count();
		std::cout << "<<< transer " << time["transer"] << "s" << std::endl;
	}
	std::cout << "<<< applicationer " << ((sec)(clk::now() - t0)).count() << "s" << std::endl;
